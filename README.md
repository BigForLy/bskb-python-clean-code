...
```
4. Нельзя передавать конструкции с *if*, *import*, *def*, *class*, *for*, *while*, однако можно использовать *for* для например list comprehension и тернарные операторы например *if* в качестве однострочного *if* 
```python
True if ... else False
```
5. Каждый тест должен быть потоконезависимый, и не должен зависеть от других тестов

# Правила формирования commit

1. Указать тип изменения:
*fix* - исправляет баги в коде,
*feat* - добавляет новый функционал
3. Указать номер КС из redmine
